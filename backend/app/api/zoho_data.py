...
#     Preserves table structure and ensures only clean, minimal fields are saved.
#     """
...
#                 # Include only limited data as text for embedding (no table metadata)
...
#                 # ---------------------------------------------------------------------
#                 # Clean chunk text for proper row-column saving (tabular readability)
#                 # ---------------------------------------------------------------------
#                 cleaned_chunk = c
...
#                 cleaned_chunk = cleaned_chunk.replace("\n", " ")
#                 cleaned_chunk = cleaned_chunk.replace("\\r", " ")
#                 cleaned_chunk = cleaned_chunk.replace("\r", " ")
#                 cleaned_chunk = cleaned_chunk.replace("\\t", " ")
#                 cleaned_chunk = cleaned_chunk.replace("\t", " ")
#                 cleaned_chunk = cleaned_chunk.replace("\\\\", " ")
...
#                 # Convert JSON-like sequences into readable table format if any
...
#                 # Trim and make sure each row separated clearly by newline
#                 cleaned_chunk = re.sub(r'\s*\|\s*', ' | ', cleaned_chunk)
...
# #         # ✅ Keep full prepared_docs for saving (for record/reference)
# #         cleaned_docs = []
...
# #         # ✅ Create folder path for embeddings
...
# #         # ✅ Save clean JSON (full info for reference)
...
# #         # ✅ Start embedding thread (ONLY 'text' field will be embedded)
# #         # We pass only the text list from the JSON for embedding
# #         text_only_data = [doc["text"] for doc in cleaned_docs if doc.get("text")]

# #         threading.Thread(
# #             target=run_from_json_file,
# #             args=(job_id, {"embedding_ready_path": embedding_ready_path, "texts_to_embed": text_only_data}, company_name),
# #             daemon=True
# #         ).start()
...
#         # ✅ Save embedding-ready JSON (clean, readable)
#         cleaned_docs = [
#             {
#                 "id": doc["id"],
#                 "report_link_name": doc["report_link_name"],
...
#         # ✅ Start embedding thread — embed only text
#         text_only_data = [doc["text"] for doc in cleaned_docs if doc.get("text")]
#         threading.Thread(
#             target=run_from_json_file,
#             args=(job_id, {"embedding_ready_path": embedding_ready_path, "texts_to_embed": text_only_data}, company_name),
...
#     except Exception as e:
#         import logging
#         logging.exception("Error fetching Zoho data", exc_info=e)
#         with lock:
#             job_status[job_id]["embedding_status"] = "failed"
#             job_status[job_id]["step"] = "error"
...
#     except Exception as e:
#         import logging
#         logging.exception("Embedding job failed", exc_info=e)
#         with lock:
#             job_status[job_id]["embedding_status"] = "failed"
#             job_status[job_id]["step"] = "error"
...
"""
zoho_embedding_pipeline_full.py

- Fetches Zoho data
- Cleans text while preserving row/column structure
- Prepares embedding-ready documents
