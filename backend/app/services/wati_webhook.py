...
from app.services.genai_response import (
    handle_user_query,
    detect_booking_intent_and_fields
)
from app.services.vectordb_retrive import query_pinecone_index
from app.services.wati_api_service import send_whatsapp_message_v2
from app.services.select_namespace import run_namespace_selector
...
def _process_message(msg_id: str, phone: str, text: str, response: dict):
    ...
    try:
        ...
        # Detect booking intent (uses memory internally)
        intent_data = detect_booking_intent_and_fields(
            user_id=phone,  # Use phone as user_id
            retrieved_context=context,
            query=text
        )
        
        # Handle booking request
        if intent_data.get("intent") == "book":
            ai_response = _handle_booking(intent_data, namespace, phone, text)
        else:
            # Handle general query (uses memory internally)
...
def _handle_booking(intent: dict, namespace: str, phone: str, text: str) -> str:
    """Handle booking request with validation"""
    doctor = intent.get("doctor")
    date_str = intent.get("date")
    start = intent.get("start")
    duration = intent.get("duration_min", 30)
    win_start = intent.get("window_start")
    win_end = intent.get("window_end")
    
    # Check required fields
    missing = []
    if not doctor: missing.append("doctor name")
    if not date_str: missing.append("date (YYYY-MM-DD)")
    if not start: missing.append("start time (HH:MM)")
    if not win_start: missing.append("window start (HH:MM)")
    if not win_end: missing.append("window end (HH:MM)")
    
    if missing:
        return (
            f"I need more details to book:\n"
            f"Missing: {', '.join(missing)}\n\n"
            f"Please provide: doctor name, date (YYYY-MM-DD), "
            f"and time (HH:MM in 24h format)."
        )
